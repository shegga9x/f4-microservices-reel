...
public class KafkaConstants {
    ...
    public KafkaConstants(Environment env,
            ...
            @Value("${ssh.service-name:reel}") String serviceName) {
    ...
    public void initStaticValues() {
        ...
        SSL_TRUSTSTORE_LOCATION = this.sslTruststoreLocation;
        SSL_KEYSTORE_LOCATION = this.sslKeystoreLocation;
        ...
        CONSUMER_CONCURRENCY = env.getProperty(
                "spring.cloud.stream.kafka.bindings.kafkaConsumer-in-0.consumer.concurrency", Integer.class, 3);
        AUTO_COMMIT_OFFSET = env.getProperty(
                "spring.cloud.stream.kafka.bindings.kafkaConsumer-in-0.consumer.autoCommitOffset", Boolean.class, true);
        START_OFFSET = env.getProperty("spring.cloud.stream.kafka.bindings.kafkaConsumer-in-0.consumer.startOffset",
                "latest");
        ...
        PARTITION_COUNT = env.getProperty(
                "spring.cloud.stream.kafka.bindings.kafkaProducer-out-0.producer.partitionCount", Integer.class, 3);
        SYNC_PRODUCER = env.getProperty("spring.cloud.stream.kafka.bindings.kafkaProducer-out-0.producer.sync",
                Boolean.class, false);
        BUFFER_SIZE = env.getProperty("spring.cloud.stream.kafka.bindings.kafkaProducer-out-0.producer.bufferSize",
                Integer.class, 16384);

        LOG.info("Initialized Kafka static constants for service: {}, topic-in: {}", SERVICE_NAME, TOPIC_NAME_IN);
        LOG.info("Initialized Kafka static constants for service: {}, topic-out: {}", SERVICE_NAME, TOPIC_NAME_OUT);
    }
}
